      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <UploadImg ref="Img1" @onSuccess="onSuccess1"></UploadImg>
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <UploadImg ref="Img2" @onSuccess="onSuccess2"></UploadImg>
        </el-form-item>
...
import { getUserInfpApi, getPersonalDetail, updatePersonal } from '@/api'
...
export default {
  ...
  methods: {
    ...
    async getUserInfpApi() {
      this.userInfo = await getUserInfpApi(this.userId)
      this.formData = await getPersonalDetail(this.userId)
      this.$refs.Img1.filelist.push({
        url: this.userInfo.staffPhoto,
      })
      this.$refs.Img2.filelist.push({
    ...
    async updatePersonal() {
      if (this.$refs.Img2.loading) {
        return this.$message.error('图片正在上传')
      }
      await updatePersonal(this.formData, this.userId)
